    ...
    switch (props[i].inputType) {
        ...
        if (props[i].languages) {
          ...
          for (let j = 0; j < themes.length; j++) {
            ...
            if (
              ...
              const value = prevData[props[i].key]?.[currentTheme] || props[i]?.defaultValue?.[currentTheme] || ''
      ...
      case 'link':
        if (props[i].languages) {
          for (let j = 0; j < languages.length; j++) {
            const titleValue =
              prevData[props[i].key]?.title?.[languages[j]] || props[i]?.defaultValue?.title?.[languages[j]] || ''

            const linkValue =
              prevData[props[i].key]?.link?.[languages[j]] || props[i]?.defaultValue?.link?.[languages[j]] || ''
          ...
          const titleValue = prevData[props[i].key]?.title || props[i]?.defaultValue?.title || ''

          const linkValue = prevData[props[i].key]?.link || props[i]?.defaultValue?.link || ''
      ...
      case 'color':
        if (props[i].themeBased && themes.length > 0) {
          for (let j = 0; j < themes.length; j++) {
            const currentTheme = themes[j]
            if (
              (prevData[props[i].key] && typeof prevData[props[i].key] === 'string') ||
      ...
      case 'flipBookPdf':
        for (let j = 0; j < languages.length; j++) {
          obj[props[i].key] = {
            ...obj[props[i].key],
            [languages[j]]: prevData[props[i].key]?.[languages[j]] || props[i]?.defaultValue?.[languages[j]] || [],
          }
        }
        break
      case 'imageUpload':
