...
handler.get(async (req, res) => {
  ...
  const widgetURL = `https://cdn.mts-online.com/${client}/${product}${v}`
  const widgetURLCloud = `https://vigilant-bose-23636c.netlify.app/${client}/${product}${v}`

  console.log({ widgetURL })
  // const widgetURL = `https://vigilant-bose-23636c.netlify.app/${client}/${product}${v}`
  ...
  const widgetsFetch = async (retry = false) => {
    console.log('====================')
    console.log(`calling ${!retry ? widgetURLCloud : widgetURL}`)
    return await fetch(!retry ? widgetURLCloud : widgetURL)
      .then((res) => {
        return { data: res.text(), error: null }
      })
      .catch((error) => {
        return !retry ? widgetsFetch(true) : { data: null, error: 'no widgets found' }
      })
  }

  const { data: widgetsText, error } = await widgetsFetch()
  console.log({ widgetsText })
  if (error) {
    res.statusCode = 404
